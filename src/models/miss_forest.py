...
class MissForest:
    ...
    * [3] https://scikit-learn.org/stable/modules/generated/sklearn.ensemble.
    ...
    def _miss_forest(self, Ximp, mask, X_test, mask_test):
        ...
        missing_rows_test, missing_cols_test = np.where(mask_test)
        ...
        if self.num_vars_ is not None:
            ...
            keep_idx_num = np.in1d(missing_cols_test, self.num_vars_)
            missing_num_rows_test = missing_rows_test[keep_idx_num]
            missing_num_cols_test = missing_cols_test[keep_idx_num]
            ...
            X_test[missing_num_rows_test, missing_num_cols_test] = np.take(
                col_means, missing_num_cols_test)
        ...
        if self.cat_vars_ is not None:
            ...
            keep_idx_cat = np.in1d(missing_cols_test, self.cat_vars_)
            missing_cat_rows_test = missing_rows_test[keep_idx_cat]
            missing_cat_cols_test = missing_cols_test[keep_idx_cat]
            ...
            X_test[missing_cat_rows_test, missing_cat_cols_test] = np.take(col_modes, missing_cat_cols_test)
        ...
        while (gamma_new < gamma_old or gamma_newcat < gamma_oldcat) and self.iter_count_ < self.max_iter:
            ...
            for s in misscount_idx:
                ...
                mis_rows_test = np.where(mask_test[:, s])[0]

                # If no missing, then skip
                if len(mis_rows) == 0 and len(mis_rows_test) == 0:
                    continue
                ...
                xmis_test = X_test[np.ix_(mis_rows_test, s_prime)]
                ...
                if self.cat_vars_ is not None and s in self.cat_vars_:
                    ...
                    if len(xmis_test) > 0:
                        ymis_test = rf_classifier.predict(xmis_test)
                        X_test[mis_rows_test, s] = ymis_test
                else:
                    ...
                    if len(xmis) > 0:
                        ...
                        Ximp[mis_rows, s] = ymis
                    if len(xmis_test) > 0:
                        ymis_test = rf_regressor.predict(xmis_test)
                        X_test[mis_rows_test, s] = ymis_test
    ...
    def fit(self, X, y=None, cat_vars=None):
        ...
        col_modes = mode(X[:, cat_vars], axis=0, nan_policy='omit')[0] if cat_vars is not None else None
    ...
    def transform(self, X, X_test=None):
        ...
        # Check for +/- inf
        if np.any(np.isinf(X)) or np.any(np.isinf(X_test)):
            raise ValueError("+/- inf values are not supported.")

        # Check if any column has all missing
        mask = _get_mask(X, self.missing_values)
        mask_test = _get_mask(X_test, self.missing_values)
        if np.any(mask.sum(axis=0) >= (X.shape[0])):
            raise ValueError("One or more columns have all rows missing.")
        ...
        X_test = self._miss_forest(X, mask, X_test, mask_test)

        # Return imputed dataset
        return X_test

    def fit_transform(self, X, X_test=None, y=None, **fit_params):
        ...
        return self.transform(X, X_test)
